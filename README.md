...
The SPDX 2.3 JSON inventory is named
`wineforge-engine-VERSION-TARGET.spdx.json`. Download it from the corresponding
`engine-reference-VERSION-TARGET` Actions artifact (retained for 14 days), or
from an approved GitHub release. It scans the staged runtime, not the entire
build host. Native libraries may not all be identifiable by the scanner; the
dependency-version evidence complements this inventory. Basic document checks
run before attestation and upload. No SBOM is generated by validation-only PR
checks, and existing release assets are not changed retroactively.
